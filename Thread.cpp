...
Thread::Thread(void *(*_threadRoutine) (void *))
{
    ...
    struct timeval thread_name;
    gettimeofday(&thread_name, NULL);
     long milliseconds = thread_name.tv_sec * 1000 + thread_name.tv_usec / 1000;
  // Get time snapshot in milli-seconds
    memset ( identifier,0,128); // Initialize identifier
    sprintf(identifier, "%ld.%03ld", thread_name.tv_sec, thread_name.tv_usec / 1000);
